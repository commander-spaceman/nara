...
pub fn process_mono_f32(
    ...
    let mut wet = if params.pitch_semitones.abs() > f32::EPSILON {
        ...
        let shifted = pitch::pitch_shift(samples, sample_rate, params.pitch_semitones);
        t_pitch = t0.elapsed().as_secs_f64() * 1_000.0;
        shifted
    } else {
        t_pitch = 0.0;
        samples.to_vec()
    ...
    let t_filters = if params.hpf > 0.0 || params.lpf > 0.0 || params.notch > 0.0 {
        let t0 = std::time::Instant::now();
        if params.hpf > 0.0 {
